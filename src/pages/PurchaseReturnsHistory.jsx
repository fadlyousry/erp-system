    ...
    <div className="sales-modal-overlay" onClick={onClose}>
      <div className="sales-modal" onClick={(event) => event.stopPropagation()}>
        <div className="sales-modal-header">
          <h2>تفاصيل مرتجع المشتريات #{formatInteger(returnInvoice.id)}</h2>
        ...
        <div className="sales-modal-meta">
          ...
          <div><strong>فاتورة المشتريات:</strong> {returnInvoice.purchaseId ? `#${formatInteger(returnInvoice.purchaseId)}` : '-'}</div>
...
export default function PurchaseReturnsHistory() {
  ...
  const purchaseReturnsSummary = useMemo(() => (
    visibleReturns.reduce((accumulator, returnInvoice) => ({
      ...
      linkedInvoices: accumulator.linkedInvoices + (returnInvoice.purchaseId ? 1 : 0)
    }), {
      invoices: 0,
      totalAmount: 0,
      itemsCount: 0,
              ...
              {visibleReturns.length === 0 && hasLoadedOnce ? (
                ...
                visibleReturns.map((returnInvoice) => (
                  <tr key={returnInvoice.id}>
                    ...
                    <td>{returnInvoice.purchaseId ? `#${formatInteger(returnInvoice.purchaseId)}` : '-'}</td>
            ...
            {visibleReturns.length > 0 && (
              <tfoot className="sales-table-footer">
                <tr className="sales-table-summary-row">
                  <td colSpan={4}></td>
                  <td className="sales-money sales-total">{formatMoney(purchaseReturnsSummary.totalAmount)}</td>
                  <td style={{ fontWeight: 'bold' }}>{formatInteger(purchaseReturnsSummary.itemsCount)}</td>
                  <td colSpan={2}></td>
                </tr>
              </tfoot>
            )}
